// DeepSeek AI client. SERVER-SIDE ONLY — never import into a browser/client
// component. Reads DEEPSEEK_API_KEY from the environment at call time.
...
function makeRunId(): string {
  ...
  return `deepseek-${Date.now()}-${rand}`;
}

function assertServerSide() {
  if (typeof window !== "undefined") {
    throw new Error("DeepSeekClient must not be used in the browser. AI calls are server-side only.");
  }
}
...
export class DeepSeekClient implements AIClient {
  ...
  /** Override the API key from org settings. Falls back to DEEPSEEK_API_KEY env var. */
  setApiKey(key: string) {
    this._apiKey = key;
  }

  private get apiKey(): string | undefined {
    return this._apiKey || process.env.DEEPSEEK_API_KEY;
  }
  ...
  async analyse(request: AIAnalysisRequest): Promise<AIAnalysisResult> {
    ...
    if (!apiKey || apiKey.trim() === "") {
      throw new Error(
        "DeepSeek API key not configured. Set it in Settings or DEEPSEEK_API_KEY in .env.",
      );
    }
    ...
    if (!message) {
      throw new Error("DeepSeek API returned no content to parse.");
    }
    ...
    try {
      ...
      throw new Error(`DeepSeek API returned non-JSON content: ${message.slice(0, 200)}`);
    }
